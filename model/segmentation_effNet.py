...
class EFNet():
    ...
    def getEFNet(self):
        ...
        weights : None(무작위 초기화), 'imagenet'(ImageNet에 대한 사전 교육) 또는 로드할 가중치 파일의 경로 중 하나입니다. 기본값은 '이미지넷'입니다.
        ...
        pooling : 기능 추출을 위한 선택적 풀링 모드 include_top입니다 False. 기본값은 없음입니다. - None모델의 출력이 마지막 컨볼루션 레이어의 4D 텐서 출력임을 의미합니다. - avg전역 평균 풀링이 마지막 컨볼루션 레이어의 출력에 적용되므로 모델의 출력은 2D 텐서가 됩니다. - max전역 최대 풀링이 적용됨을 의미합니다.
        classes : 이미지를 분류할 선택적 클래스 수이며 include_top, True인 경우와 weights인수가 지정되지 않은 경우에만 지정됩니다. 기본값은 1000(ImageNet 클래스 수)입니다.
        classifier_activation : A str또는 호출 가능. "최상위" 레이어에서 사용할 활성화 함수입니다. 가 아니면 무시됩니다 include_top=True. classifier_activation=None"최상위" 레이어의 로짓을 반환하도록 설정 합니다. 기본값은 'softmax'입니다. 사전 훈련된 가중치를 로드할 때 는 또는 classifier_activation이어야 합니다 .None"softmax"
        """
        effnet = efn.EfficientNetB7(weights=None,
                                    include_top=False,
